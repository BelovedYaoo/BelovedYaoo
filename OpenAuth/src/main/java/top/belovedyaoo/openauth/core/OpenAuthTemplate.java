package top.belovedyaoo.openauth.core;
...
import top.belovedyaoo.openauth.data.model.CodeModel;
...
public class OpenAuthTemplate {
    ...
    public OpenAuthClientModel checkClientSecret(String clientId, String clientSecret) {
        ...
        OpenException.throwBy(cm.clientSecret == null || !cm.clientSecret.equals(clientSecret), OidcExceptionEnum.INVALID_CLIENT_SECRET, clientId, clientSecret);
        return cm;
    }
    ...
    public OpenAuthClientModel checkContractScope(OpenAuthClientModel cm, List<String> scopes) {
        ...
        for (String scope : scopes) {
            OpenException.throwBy(!cm.contractScopes.contains(scope), OpenAuthExceptionEnum.PERMISSION_DENIED, "该Client未签约该Scope权限", cm.clientId, scope);
        }
        return cm;
    }
    ...
    public void checkRedirectUri(String clientId, String url) {
        // 1、是否是一个有效的url
        OpenException.throwBy(!SaFoxUtil.isUrl(url), OidcExceptionEnum.INVALID_REDIRECT_URI, url);
        ...
        //
        //  但是为了安全起见，这么做还是有必要的
        OpenException.throwBy(url.contains("@"), OidcExceptionEnum.INVALID_REDIRECT_URI, url);

        // 4、是否在[允许地址列表]之中
        OpenAuthClientModel clientModel = checkClientModel(clientId);
        checkRedirectUriListNormal(clientModel.allowRedirectUris);
        OpenException.throwBy(!SaStrategy.instance.hasElement.apply(clientModel.allowRedirectUris, url), OidcExceptionEnum.INVALID_REDIRECT_URI, url, clientId);
    }
    ...
    public static void checkRedirectUriListNormalStaticMethod(List<String> redirectUriList) {
        for (String url : redirectUriList) {
            ...
            //
            //  但是为了安全起见，这么做还是有必要的
            OpenException.throwBy(index != -1 && index != url.length() - 1, OidcExceptionEnum.INVALID_REDIRECT_URI, url);
        }
    }
    ...
    public CodeModel checkGainTokenParam(String code, String clientId, String clientSecret, String redirectUri) {
        ...
        // 校验：ClientId是否一致
        OpenException.throwBy(!cm.clientId.equals(clientId), OidcExceptionEnum.INVALID_CLIENT_ID, clientId);

        // 校验：Secret是否正确
        String dbSecret = checkClientModel(clientId).clientSecret;
        OpenException.throwBy(dbSecret == null || !dbSecret.equals(clientSecret), OidcExceptionEnum.INVALID_CLIENT_SECRET, clientId, clientSecret);

        // 如果提供了redirectUri，则校验其是否与请求Code时提供的一致
        OpenException.throwBy(!SaFoxUtil.isEmpty(redirectUri), OidcExceptionEnum.UNCONFORMITY_REDIRECT_URI, redirectUri, clientId);
    ...
    public AccessTokenModel checkAccessTokenParam(String clientId, String clientSecret, String accessToken) {
        ...
        OpenException.throwBy(!at.clientId.equals(clientId), OidcExceptionEnum.UNCONFORMITY_CLIENT_ID, clientId);
    ...
    public AccessTokenModel checkAccessToken(String accessToken) {
        ...
        OpenException.throwBy(at == null, OidcExceptionEnum.INVALID_ACCESS_TOKEN, accessToken);
        return at;
    }
    ...
    public void checkAccessTokenScope(String accessToken, String... scopes) {
        ...
        for (String scope : scopes) {
            OpenException.throwBy(!at.scopes.contains(scope), OpenAuthExceptionEnum.PERMISSION_DENIED, "该AccessToken不具备该Scope权限", accessToken, scope);
        }
    }
    ...
    public ClientTokenModel checkClientToken(String clientToken) {
        ...
        OpenException.throwBy(ct == null, OidcExceptionEnum.INVALID_CLIENT_TOKEN, clientToken);
        return ct;
    }
    ...
    public void checkClientTokenScope(String clientToken, String... scopes) {
        ...
        for (String scope : scopes) {
            OpenException.throwBy(!ct.scopes.contains(scope), OpenAuthExceptionEnum.PERMISSION_DENIED, "该ClientToken不具备该Scope权限", clientToken, scope);
        }
    }
